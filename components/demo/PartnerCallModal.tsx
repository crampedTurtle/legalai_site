...
export function PartnerCallModal() {
  const { isOpen, close, source } = usePartnerCallModal()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const id = await submitLead({
        ...
        notes: `Partner Call Request - State: ${formData.state}, Referral Experience: ${formData.referralExperience}, Notes: ${formData.notes}`,
        wants_demo: false,
        source: source || 'partner:call',
      }, 'partner_call')
      setLeadId(id)
      setSuccess(true)
    } catch (err) {
      console.error('Failed to submit partner call request:', err)
        ...
        <div className="card-hover p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          {!loadingSlots && slots.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-dark-400">We'll be in touch to schedule your partner call manually.</p>
            </div>
          )}
          ...
          {booking && (
            <div className="mt-6 p-4 bg-green-500/20 border border-green-500/30 rounded-lg">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <h4 className="font-medium text-white">Partner Call Scheduled!</h4>
              </div>
              <p className="text-dark-300 text-sm mb-3">
                We've scheduled your partner call. You'll receive a confirmation email shortly.
              </p>
              {booking?.htmlLink && (
                <a 
          ...
          <div className="mt-6 text-center">
            <Button 
              onClick={() => {
                ...
                setSlots([])
                setBooking(null)
                setFormData({ firstName: '', lastName: '', email: '', company: '', title: '', phone: '', state: '', referralExperience: '', notes: '' })
                // Close the modal
                close()
              }}
              variant="outline"
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="card-hover p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-dark-300 mb-6">
          Let's discuss how we can work together to help law firms adopt private AI. 
          Tell us about your background and we'll schedule a call.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-dark-800 border border-dark-700 rounded-lg text-white placeholder-dark-400 focus:outline-none focus:ring-2 focus:ring-sapphire-500 focus:border-transparent transition-colors"
                placeholder="Legal Tech Solutions"
                disabled={loading}
              />
            </div>
            <div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="state" className="block text-sm font-medium text-dark-300 mb-2">
                State/Region
              </label>
              <input
                type="text"
          ...
          <div>
            <label htmlFor="referralExperience" className="block text-sm font-medium text-dark-300 mb-2">
              Referral Experience
            </label>
            <select
              id="referralExperience"
            ...
            >
              ...
              <option value="New to referrals">New to referrals</option>
              <option value="Some referral experience">Some referral experience</option>
              <option value="Experienced referrer">Experienced referrer</option>
              <option value="Professional referral partner">Professional referral partner</option>
            </select>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="What types of law firms do you work with? What are your partnership goals? Any specific questions about our referral program?"
              disabled={loading}
            />
          </div>

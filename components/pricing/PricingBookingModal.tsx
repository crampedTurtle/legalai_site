...
export function PricingBookingModal({ isOpen, onClose, source = 'pricing' }: PricingBookingModalProps) {
  ...
  const meetingTypes = [
    { value: '15min', label: '15 Min Meeting', duration: '15m' },
    { value: '30min', label: '30 Min Meeting', duration: '30m' },
    { value: 'demo', label: 'Demo', duration: '25m' }
  ]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const id = await submitLead({
        ...
        notes: `Pricing ${formData.meetingType} meeting - ${formData.notes}`,
        wants_demo: formData.meetingType === 'demo',
        source: `${source}:${formData.meetingType}`,
      }, 'pricing_meeting')

      setLeadId(id)
      setIsSuccess(true)
    } catch (err) {
      console.error('Failed to submit pricing meeting request:', err)
          ...
          {isSuccess ? (
            <div className="text-center py-8">
              ...
              {!loadingSlots && slots.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-dark-400">We'll be in touch to schedule your meeting manually.</p>
                </div>
              )}
              ...
              {booking && (
                <div className="mt-6 p-4 bg-green-500/20 border border-green-500/30 rounded-lg">
                  <div className="flex items-center space-x-3 mb-2">
                    ...
                    <h4 className="font-medium text-white">Meeting Scheduled!</h4>
                  </div>
                  <p className="text-dark-300 text-sm mb-3">
                    We've scheduled your {formData.meetingType} meeting. You'll receive a confirmation email shortly.
                  </p>
                  {booking?.htmlLink && (
                    <a 
            ...
            <div>
              <DialogDescription className="text-dark-300 mb-6">
                Tell us about your needs and we'll schedule a meeting to discuss how Sapphire Legal AI can help your firm.
              </DialogDescription>
